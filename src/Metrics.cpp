...
inline std::pair<std::pair<double, double>, std::map<char, std::pair<double, double>>> computePrecisionRecall(
  ...
  std::map<char, std::pair<double, double>> robot_precision_recall;
  ...
  for (auto& rid : dataset.robots()) {
    ...
    for (size_t i = 0; i < stop_idx; i++) {
      ...
      for (auto& potential_outlier_status : entry.potential_outlier_statuses) {
        auto& [factor_idx, is_outlier] = potential_outlier_status;
        bool pred_outlier = (*results.robot_outliers)[rid].find(factor_idx) != (*results.robot_outliers)[rid].end();
